n_fold_cross_validation <-
   function(data,
      ...
      for (i in 1:n_repetition) {
         ...
         accuracy_for_each_iteration <-
            matrix(0, nrow = n_fold, ncol = 3)
         for (j in 1:n_fold) {
            ...
            data_new_test <-
               scale(test,
                     ...
                     scale = pca_result$scale) %*% pca_result$eigenfaces
            pca_result$new_test <-
               data.frame(labels = test_label, data = data_new_test)
            for (k in 1:3) {
               accuracy_for_each_iteration[j, k] <-
                  classify(pca_result$new_train,
                           pca_result$new_test,
                           k,
                           pca_result$eigenvalues) * 100
            }
         }
      ...
      accuracy <-
         data.frame(accuracy,
                    row.names = c("Euclidean", "Mahalanobis", "Manhattan"))
      colnames(accuracy) = paste0("Accuracy(%) of rep#", seq_len(ncol(accuracy)))
      return(accuracy)
